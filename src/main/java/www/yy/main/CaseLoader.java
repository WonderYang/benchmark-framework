...
import www.yy.main.cases.SortCase;
...
class CaseRunner {
    ...
    public void runCase(Case bCase) throws Exception{
        ...
        for(Method method : methods) {
            ...
            Annotation measurement = method.getAnnotation(Measurement.class);
            //如果测试方法中就有设置好的注解参数，我们将他读取出来
            if(measurement != null) {
                iterations = ((Measurement) measurement).iterations();
                group = ((Measurement) measurement).group();
            }
            //预热部分
            Annotation warmUp = method.getAnnotation(WarmUp.class);
            if(warmUp != null) {
                iterationsWarmUp = ((WarmUp) warmUp).iterations();
            }
            //再调用具体实现的方法
    ...
    public void run(Case bCase, Method method, int iterations, int group, int iterationsWarmUp) throws Exception{
        ...
        for (int i=0; i<group; i++) {
            ...
            long timeEnd = System.nanoTime();
            long time = timeEnd - timeStart;
            System.out.println("耗时："+time/iterations);
        }
        System.out.println("#############################################################");
    }
}
public class CaseLoader {
    ...
    //返回一个List集合，集合中包含了所有测试类的名字，即类名称以Case为结尾的类名称，目的是得到这些名字，就能
